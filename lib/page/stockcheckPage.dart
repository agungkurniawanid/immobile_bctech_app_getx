...
class StockCheckPage extends StatefulWidget {
  @override
  _StockCheckPage createState() => _StockCheckPage();
}

class _StockCheckPage extends State<StockCheckPage> {
  bool _allow = true;
                            ...
                            child: RichText(
                              text: TextSpan(
                                ...
                                children: [
                                  ...
                                  TextSpan(
                                    text: stock.formatted_updated_at
                                            .contains("Today")
  ...
  void searchWF(String search) async {
    stockcheckVM.toliststock.value.clear();
    search = search.toUpperCase();

    var locallist2 = liststockmodel
        .where((element) => element.location.contains(search))
        .toList();

    for (var i = 0; i < locallist2.length; i++) {
      stockcheckVM.toliststock.value.add(locallist2[i]);
    }
  }
  ...
  void _startSearch() {
    setState(() {
      ...
      var locallist = stockcheckVM.toliststock.value;
  ...
  void _clearSearchQuery() {
    setState(() {
      ...
      for (var item in liststockmodel) {
        stockcheckVM.toliststock.value.add(item);
      }
                        ...
                        child: Obx(() {
                          return Expanded(
                            child: ListView.builder(
                                ...
                                //         crossAxisCount: 1),
                                itemCount:
                                    stockcheckVM.toliststock.value.length,
                                itemBuilder: (BuildContext context, int index) {
                                  return GestureDetector(
                                    child: headerCard2(
                                        stockcheckVM.toliststock.value[index]),
                                    onTap: () async {
                                      Get.to(DetailStockCheckPage(
                                          index, "stockcheckpage"));
                                    },
                                  );
                                }),
                          );

...
class _DetailPidPageState extends State<DetailPidPage> {
  ...
  Future<void> _sendDataWedgeCommand(String command, String parameter) async {
    try {
      final argumentAsJson = jsonEncode({
        "command": command,
        "parameter": parameter,
      });
      ...
      debugPrint('⚠️ Gagal kirim DataWedge command: ${e.message}');
    } catch (e) {
      debugPrint('❌ Error: $e');
    }
  }
  ...
  Future<void> _createProfile(String profileName) async {
    try {
      ...
      debugPrint('⚠️ Gagal membuat profil: ${e.message}');
    } catch (e) {
      debugPrint('❌ Error: $e');
    }
  }
  ...
  void _onEvent(event) {
    setState(() {
      ...
      final String barcodeString = barcodeScan['scanData'] ?? "";
    ...
    try {
      ...
      if (barcodeScanRes != "-1" && barcodeScanRes.isNotEmpty) {
        ...
        if (details.isNotEmpty) {
          try {
            foundItem = details.firstWhere(
              (element) => element.itemCode?.contains(barcodeScanRes) ?? false,
            );
          } catch (_) {
            foundItem = null;
          }
        ...
        if (foundItem != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text("Barcode tidak ditemukan dalam stok."),
            ),
          );
        }
      }
    } catch (e) {
      debugPrint("Barcode scan dibatalkan atau error: $e");
    }
  }

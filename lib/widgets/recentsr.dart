...
class RecentSR extends StatelessWidget {
  ...
  int calcuforcolour() {
    int ab = stockrequestVM.tolistsrout[index].detail
        .where((element) =>
            element.approvename == "" && element.inventory_group == "AB")
        .toList()
        .length;
    int ch = stockrequestVM.tolistsrout[index].detail
        .where((element) =>
            element.approvename == "" && element.inventory_group == "CH")
        .toList()
        .length;
    int fz = stockrequestVM.tolistsrout[index].detail
        .where((element) =>
  ...
  String _calcuCTN() {
    try {
      ...
      if (stockrequestVM.choicesr.value == "ALL") {
        ...
        listbyinventorygroup = stockrequestVM.tolistsrout[index].detail
            .where((element) =>
                element.inventory_group == stockrequestVM.choicesr.value)
            .toList();
      }
                ...
                child: RichText(
                  text: TextSpan(
                    ...
                    children: [
                      ...
                      TextSpan(
                        text: '${stockrequestVM.tolistsrout[index].total_item}',
                        style: SafeGoogleFont(
                          'Roboto',
                          fontSize: 12 * ffem,
                          fontWeight: FontWeight.w400,

...
import 'package:immobile_app_fixed/models/in_detail_model.dart';
import 'package:immobile_app_fixed/models/in_model.dart';
import 'package:immobile_app_fixed/models/item_choice_model.dart';
import 'package:immobile_app_fixed/pages/mydialogPage.dart';
...
class _InDetailPageState extends State<InDetailPage>
  ...
  void initState() {
    ...
    if (widget.from == "sync") {
      ...
      containerinput.text = inVM.tolistPO[widget.index].truck ?? '';
    }
  ...
  void _onEvent(dynamic event) {
    setState(() {
      final Map<String, dynamic> barcodeScan = jsonDecode(event);
  ...
  Future<void> scanBarcode() async {
    ...
    if (result != '-1' && result.isNotEmpty) {
      ...
      final tData = inVM.tolistPO[widget.index].tData ?? [];
      final matchedBarcode = tData
          .where((e) => e.barcode?.contains(result) ?? false)
          .toList();

      if (matchedBarcode.isEmpty) return;

      final data = matchedBarcode.first;

      pcs.value = (data.qtuom ?? 0).toInt();
      ctn.value = data.qtctn ?? 0;
      kg.value = (data.qtuom ?? 0).toDouble();

      typeIndexctn = ctn.value;
      typeIndexpcs = pcs.value;
      typeIndexkg = kg.value;
      expireddate.value = data.vfdat ?? '';
      checkingscan = true;
